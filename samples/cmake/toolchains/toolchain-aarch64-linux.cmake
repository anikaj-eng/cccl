...
set(
  CMAKE_CUDA_FLAGS
  "${CMAKE_CUDA_FLAGS} -ccbin ${CMAKE_CXX_COMPILER}"
  CACHE STRING
  ""
  FORCE
)
...
if (DEFINED TARGET_FS)
  ...
  foreach (lib_path ${LIB_PATHS})
    set(
      ...
      "${CMAKE_EXE_LINKER_FLAGS} -Wl,-rpath-link,${lib_path}"
    )
    set(
      CMAKE_SHARED_LINKER_FLAGS
      "${CMAKE_SHARED_LINKER_FLAGS} -Wl,-rpath-link,${lib_path}"
    )
  endforeach()

  # Add the real path of CUDA installation on TARGET_FS for nvvm
  find_program(
    TARGET_CUDA_NVCC_PATH
    nvcc
    PATH
    "${TARGET_FS}/usr/local/cuda/bin"
    NO_DEFAULT_PATH
    NO_CMAKE_PATH
  )
  ...
  if (DEFINED TARGET_CUDA_ROOT)
    list(
      ...
      "${TARGET_CUDA_ROOT}/targets/${CUDA_AARCH64_TARGET}/lib"
    )
